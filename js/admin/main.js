        ...
        var sql=window.location.search.substr(1);
            if(sql.length!=0){
                $("body > div.btn-toolbar").css({"display": "none"});
                $("body > bookmark").css({"display": "none"});
           ...
           // console.log(tab);
            ...
            $("input[name=col]").change(function(){ 
                 ...
                 var data={setSort: true,setSort_val: $("#sort_po").val()}
                   tab.ajax(data);   
                
                 var data={setPoiscDiapason: true,setPoiscDiapason_val: $("#vikl").val()}
                    $.ajax({
                        type: "POST",
            ...
            $("input[name=sort_date]").change(function(){               
                   var data={setSort: true,setSort_val: $(this).val()}
                   tab.ajax(data);               
            });
              ...
              $("input[name=dat]").change(function(){
                  if($(this).val()=='diapason'){     //если выбран диапазон, открываем модальное окно
                     $('#myModal').modal('show'); 
                       ...
                       var data={setPoiscDiapason: true,setPoiscDiapason_val: $(this).val()}
                        $.ajax({
                            type: "POST",
                            url: "ajax/admin/for_admin.php",
                            cache: false,
            ...
            /*====================================ПОИСК=====================================*/
                   $('#v_poisc').on('input',function() {
                        ...
                        var data={setPoisc: true,poisc: poisc,val_poisc:  val_poisc}
                         $.ajax({
                            type: "POST",
                            url: "ajax/admin/for_admin.php",
                            cache: false,
             ...
             $("#start_d").click(function(){
                 ...
                 if(s.length<=0 & f.length<=0){
                     ...
                     var data={setPoiscDiapason: true,setPoiscDiapason_val: zapros}
                        $.ajax({
                            type: "POST",
                            url: "ajax/admin/for_admin.php",
                            cache: false,
